...
# ---------------------------------------------------------------------------
# Fused Triton kernel for Rotary Position Embedding on Mamba2 B/C tensors.
# Applied to (batch, seqlen, K*ngroups, d_state) projections before the scan.
# Each program handles one (batch, seq_pos, group) slice of size d_state.
# 'inverse=True' rotates by -θ, used to un-rotate gradients dB and dC.
# ---------------------------------------------------------------------------
@triton.jit
def _rope_bc_kernel(
    B_ptr, C_ptr,
    ...
    mask = offs < half
    freq = tl.exp(-log_theta_base * (offs * 2).to(tl.float32) / D_STATE)
    angle = l_pos.to(tl.float32) * freq
    cos_a = tl.cos(angle)
    sin_a = tl.sin(angle) if not INVERSE else -tl.sin(angle)
    base = pid * D_STATE
    b_r = tl.load(B_ptr + base + offs * 2,     mask=mask, other=0.0).to(tl.float32)
    b_i = tl.load(B_ptr + base + offs * 2 + 1, mask=mask, other=0.0).to(tl.float32)
    tl.store(B_ptr + base + offs * 2,     b_r * cos_a - b_i * sin_a, mask=mask)
    tl.store(B_ptr + base + offs * 2 + 1, b_r * sin_a + b_i * cos_a, mask=mask)
    c_r = tl.load(C_ptr + base + offs * 2,     mask=mask, other=0.0).to(tl.float32)
    c_i = tl.load(C_ptr + base + offs * 2 + 1, mask=mask, other=0.0).to(tl.float32)
    tl.store(C_ptr + base + offs * 2,     c_r * cos_a - c_i * sin_a, mask=mask)
    tl.store(C_ptr + base + offs * 2 + 1, c_r * sin_a + c_i * cos_a, mask=mask)


def _rope_apply_bc(B: Tensor, C: Tensor, rope_theta: float, inverse: bool = False):
    """Apply (or invert) RoPE to B and C in-place on float32 copies.
    B, C: (batch, seqlen, K*ngroups, d_state) — d_state must be even.
    Returns new float32 tensors with the same original dtype restored.
    """
    orig_dtype = B.dtype
    B_f = B.float().contiguous()
    C_f = C.float().contiguous()
    bsz, L, KG, d_state = B_f.shape
    half = d_state // 2
    block_half = triton.next_power_of_2(half)
    total_slices = bsz * L * KG
    log_theta_base = math.log(rope_theta)
    _rope_bc_kernel[(total_slices,)](
        B_f, C_f, total_slices, L, KG, log_theta_base,
        D_STATE=d_state, BLOCK_HALF=block_half, INVERSE=inverse,
    )
    return B_f.to(orig_dtype), C_f.to(orig_dtype)


def _mamba_chunk_scan_combined_fwd(x, dt, A, B, C, chunk_size, D=None, z=None, dt_bias=None, initial_states=None, seq_idx=None, cu_seqlens=None, dt_softplus=False, dt_limit=(0.0, float("inf")), rope_theta=None):
    batch, seqlen, nheads, headdim = x.shape
    _, _, ngroups, dstate = B.shape
    assert nheads % ngroups == 0
    assert B.shape == (batch, seqlen, ngroups, dstate)
    ...
    # Apply Rotary Position Embedding to B and C before the scan.
    # d_state must be even; uses a fused Triton kernel (one launch for both tensors).
    if rope_theta is not None:
        assert dstate % 2 == 0, f"d_state must be even for RoPE, got {dstate}"
        B, C = _rope_apply_bc(B, C, rope_theta, inverse=False)
...
def _mamba_chunk_scan_combined_bwd(dout, x, dt, A, B, C, out, chunk_size, D=None, z=None,
    ...
    dt_in = dt.clone()
    # Re-apply RoPE to B and C so backward state-recomputation sees the same rotated
    # tensors that the forward scan used. The saved B, C in ctx are the originals.
    if rope_theta is not None:
        B, C = _rope_apply_bc(B, C, rope_theta, inverse=False)
    ...
    # Invert RoPE on dB and dC: gradient w.r.t. original (unrotated) B and C.
    # RoPE is an orthogonal transform so R^{-1} = R^T = R(-theta).
    if rope_theta is not None:
        dB_given, dC_given = _rope_apply_bc(dB_given, dC_given, rope_theta, inverse=True)
...
class MambaChunkScanCombinedFn(torch.autograd.Function):
    ...
    @staticmethod
    def forward(ctx, x, dt, A, B, C, chunk_size, D=None, z=None, dt_bias=None, initial_states=None, seq_idx=None, cu_seqlens=None, dt_softplus=False, dt_limit=(0.0, float("inf")), rope_theta=None, return_final_states=False, return_varlen_states=False):
        ctx.dt_dtype = dt.dtype
        if not return_varlen_states:
            cu_seqlens = None
        else:
            assert cu_seqlens is not None, "cu_seqlens must be provided if return_varlen_states is True"
        out, out_x, dt_out, dA_cumsum, states, final_states, *rest = _mamba_chunk_scan_combined_fwd(x, dt, A, B, C, chunk_size, D=D, z=z, dt_bias=dt_bias, initial_states=initial_states, seq_idx=seq_idx, cu_seqlens=cu_seqlens, dt_softplus=dt_softplus, dt_limit=dt_limit, rope_theta=rope_theta)
    ...
    def backward(ctx, dout, *args):
        ...
        dx, ddt, dA, dB, dC, dD, dz, ddt_bias, dinitial_states = _mamba_chunk_scan_combined_bwd(dout, x, dt, A, B, C, out, ctx.chunk_size, D=D, z=z, dt_bias=dt_bias, initial_states=initial_states, dfinal_states=dfinal_states, seq_idx=seq_idx, dt_softplus=ctx.dt_softplus, dt_limit=ctx.dt_limit, rope_theta=ctx.rope_theta)
        # None for: chunk_size, D, z, dt_bias, initial_states, seq_idx, cu_seqlens, dt_softplus, dt_limit, rope_theta, return_final_states, return_varlen_states
        return dx, ddt, dA, dB, dC, None, dD, dz, ddt_bias, dinitial_states, None, None, None, None, None, None, None


def mamba_chunk_scan_combined(x, dt, A, B, C, chunk_size, D=None, z=None, dt_bias=None, initial_states=None, seq_idx=None, cu_seqlens=None, dt_softplus=False, dt_limit=(0.0, float("inf")), rope_theta=None, return_final_states=False, return_varlen_states=False):
    """
    Argument:
        x: (batch, seqlen, nheads, headdim)
        ...
        rope_theta: float or None. If set, apply 1-D Rotary Position Embedding to B and C
            before the scan (Mamba-3 / complex-state approach). d_state must be even.
    Return:
        out: (batch, seqlen, nheads, headdim)
    """
    return MambaChunkScanCombinedFn.apply(x, dt, A, B, C, chunk_size, D, z, dt_bias, initial_states, seq_idx, cu_seqlens, dt_softplus, dt_limit, rope_theta, return_final_states, return_varlen_states)
