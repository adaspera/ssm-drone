...
from .utils import _get_clones, bias_init_with_prob, inverse_sigmoid, linear_init

__all__ = "OBB", "Classify", "Detect", "MambaDETRDecoder", "Pose", "RTDETRDecoder", "Segment", "YOLOEDetect", "YOLOESegment", "v10Detect"
...
class MambaDecoderLayer(nn.Module):
    ...
    def __init__(self, d_model: int = 256, d_ffn: int = 1024, dropout: float = 0.0, n_levels: int = 3):
        ...
        # Bilinear-samples spatial features at reference box centers per level,
        # projects to d_model, fuses with query, then runs Mamba.
    ...
    def _sample_at_reference(self, feats_spatial: list, refer_bbox: torch.Tensor) -> torch.Tensor:
        ...
        Args:
            feats_spatial: List of (bs, hd, h, w) tensors, one per level.
            refer_bbox: (bs, nq, 4) sigmoid-normalized (cx, cy, w, h).

        Returns:
            (bs, nq, hd * n_levels) sampled context.
        """
        # Centers in [0,1] → [-1,1] for grid_sample
    ...
    def forward(
        ...
        Args:
            embed: (bs, nq, d_model) query embeddings.
            refer_bbox: (bs, nq, 4) reference bboxes in sigmoid space.
            feats_spatial: List of (bs, d_model, h, w) projected feature maps.
            query_pos: (bs, nq, d_model) optional positional embeddings.

        Returns:
            Updated query embeddings (bs, nq, d_model).
        """
        device = embed.device
        dtype = embed.dtype
        ...
        if not embed.is_cuda and not mamba_device.type == "cpu":
            return embed

        # 1. Self-Mamba (replaces self-attention)
        ...
        # 2. Cross-Mamba (replaces MSDeformAttn)
        ctx = self._sample_at_reference(feats_spatial, refer_bbox)  # (bs, nq, hd*nl)
        ctx = self.ctx_proj(ctx)  # (bs, nq, hd)
...
class MambaDecoder(nn.Module):
    ...
    Mirrors DeformableTransformerDecoder but uses Mamba layers.
    """

    def __init__(self, hidden_dim: int, decoder_layer: nn.Module, num_layers: int, eval_idx: int = -1):
        """Initialize MambaDecoder.

        Args:
            hidden_dim: Hidden dimension.
            decoder_layer: A MambaDecoderLayer instance to be cloned.
            num_layers: Number of decoder layers.
            eval_idx: Layer index to use at eval time (-1 = last).
        """
        super().__init__()
    ...
    def forward(
        ...
        """Forward pass through all decoder layers with iterative bbox refinement.

        Args:
            embed: (bs, nq, hd) initial query embeddings.
            refer_bbox: (bs, nq, 4) reference bboxes (pre-sigmoid).
            feats_spatial: List of (bs, hd, h, w) projected spatial features per level.
            bbox_head: Per-layer bbox regression heads.
            score_head: Per-layer classification heads.
            pos_mlp: MLP to compute positional encoding from reference bbox.
            attn_mask: Ignored (kept for interface compatibility).

        Returns:
            dec_bboxes: Stacked refined bboxes from each layer.
            dec_cls: Stacked class scores from each layer.
        """
        output = embed
        dec_bboxes = []
        dec_cls = []
...
class MambaDETRDecoder(nn.Module):
    """Drop-in replacement for RTDETRDecoder using Mamba instead of transformer decoder layers.

    Keeps the same overall DETR pipeline:
    - Top-k query selection via encoder scores
    - CDN denoising during training
    - Iterative bounding box refinement per layer
    - Identical training/eval output format to RTDETRDecoder

    Replaces DeformableTransformerDecoder with MambaDecoder:
    - Self-attention → Mamba1D on query sequence
    - MSDeformAttn cross-attention → bilinear sampling at reference points + Mamba1D

    YAML usage (same as RTDETRDecoder):
        - [[P3, P4, P5], 1, MambaDETRDecoder, [nc]]
    """
    ...
    def __init__(
        ...
        """Initialize MambaDETRDecoder.

        Args:
            nc: Number of classes.
            ch: Input channel sizes per feature level (inserted by tasks.py).
            hd: Hidden dimension throughout the decoder.
            nq: Number of object queries.
            ndl: Number of Mamba decoder layers.
            d_ffn: Feed-forward network hidden dimension inside each layer.
            dropout: Dropout probability.
            eval_idx: Which layer's output to use at eval time (-1 = last).
            nd: Number of denoising queries during training.
            label_noise_ratio: CDN label noise ratio.
            box_noise_scale: CDN box noise scale.
            learnt_init_query: Whether to use learned query embeddings.
        """
        super().__init__()
        self.hidden_dim = hd
        self.nl = len(ch)
        ...
        # Input projection: project each scale to uniform hd channels
        ...
        # Denoising (identical to RTDETRDecoder)
        self.denoising_class_embed = nn.Embedding(nc, hd)
        self.num_denoising = nd
        self.label_noise_ratio = label_noise_ratio
        self.box_noise_scale = box_noise_scale
    ...
    def _reset_parameters(self):
        """Initialize weights (same scheme as RTDETRDecoder)."""
        bias_cls = bias_init_with_prob(0.01) / 80 * self.nc
        constant_(self.enc_score_head.bias, bias_cls)
        constant_(self.enc_bbox_head.layers[-1].weight, 0.0)
        constant_(self.enc_bbox_head.layers[-1].bias, 0.0)
    ...
    def _generate_anchors(shapes, grid_size=0.05, dtype=torch.float32, device="cpu", eps=1e-2):
        """Generate sigmoid-space anchor boxes for all feature levels."""
        anchors = []
        for i, (h, w) in enumerate(shapes):
            sy = torch.arange(end=h, dtype=dtype, device=device)
            sx = torch.arange(end=w, dtype=dtype, device=device)
    ...
    def _get_encoder_input(self, x: list):
        """Project multi-scale features and return both flattened (for query selection) and spatial (for cross-sampling).

        Args:
            x: List of (bs, c, h, w) feature maps.

        Returns:
            feats: (bs, L, hd) concatenated flattened features.
            shapes: List of [h, w] per level.
            feats_spatial: List of (bs, hd, h, w) projected spatial features.
        """
        ...
        for proj in feats_spatial:
            h, w = proj.shape[2:]
            feats.append(proj.flatten(2).permute(0, 2, 1))  # (bs, h*w, hd)
            shapes.append([h, w])
        feats = torch.cat(feats, 1)  # (bs, L, hd)
        return feats, shapes, feats_spatial

    def _get_decoder_input(self, feats, shapes, dn_embed=None, dn_bbox=None):
        """Select top-k queries from encoder output (same as RTDETRDecoder)."""
        bs = feats.shape[0]
        if self.dynamic or self.shapes != shapes:
            self.anchors, self.valid_mask = self._generate_anchors(shapes, dtype=feats.dtype, device=feats.device)
            self.shapes = shapes
    ...
    def forward(self, x: list, batch: dict | None = None):
        ...
        Args:
            x: List of (bs, c, h, w) feature maps from the neck (P3, P4, P5).
            batch: Batch dict for CDN denoising during training.

        Returns:
            Training: (dec_bboxes, dec_scores, enc_bboxes, enc_scores, dn_meta)
            Inference: (bs, nq, 4+nc) tensor or tuple with extras.
        """
        from ultralytics.models.utils.ops import get_cdn_group
